...
class View:
    ...
    def show_menu(self):
        ...
        self.show_message('2. Generating `randomized` data (only for "Classes")')
    ...
    def show_search(self):
        self.show_message("\nSearch:")
        self.show_message("1. Which classes are taught by which instructors?")
        self.show_message("2. Average class duration time for each instructor.")
        self.show_message("3. Classes with the largest number of registered users.")
    ...
    def show_instructors_with_classes(self, rows):
        print("\nInstructors-Classes:")
        for row in rows:
            print(f"Instructor name: {row[0]}, Instructor surname: {row[1]}, Class: {row[2]}")

    def show_average_class_duration(self, rows):
        print("\nAverage class duration time for each instructor:")
        for row in rows:
            print(
                f"Instructor ID: {row[0]}, Instructor name: {row[1]}, Instructor surname: {row[2]}, Average duration: {row[3]}")
